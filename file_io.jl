...
function show_experiment_outcomes(set_nash, best_nash, statistics)
    ...
    open("$temp_dir/experiment_outcomes.txt","w") do io
        ...
        pretty_table(io, reduce(vcat, statistics_table), header = statistics_header, columns_width = [30,40,40], hlines = [0,1,2,4,5,7,8,11,12,16,17,20,21,25])
    end
    ...
    open("$temp_dir/nash_outcomes.txt","w") do io
        pretty_table(io, reduce(vcat, nash_table), header = nash_header, hlines = [0,1,4,5,11])
    end
    ...
    open("$temp_dir/class_outcomes.txt","w") do io
        pretty_table(io, reduce(vcat, class_table), header = class_header, hlines = [0,1,4,5,11])
    end
