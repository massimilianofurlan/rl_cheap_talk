...
for dir in readdir(input_dir, join = true)
	...
	config, extracted_data, babbling_nash, policies = read_data(dir)
	...
	expected_reward_s[alpha_idx,lambda_idx,:,:] .= extracted_data["expected_reward_s"]
	expected_reward_r[alpha_idx,lambda_idx,:,:] .= extracted_data["expected_reward_r"]
	mutual_information[alpha_idx,lambda_idx,:,:] .= extracted_data["mutual_information"]
	max_max_mass_on_suboptim[alpha_idx,lambda_idx,:,:] .= extracted_data["max_max_mass_on_suboptim"]
	max_absolute_error[alpha_idx,lambda_idx,:,:] .= extracted_data["max_absolute_error"]
	is_nash[alpha_idx,lambda_idx,:,:] .= extracted_data["max_max_mass_on_suboptim"] .< 1f-2 
	n_episodes[alpha_idx,lambda_idx,:,:] .= extracted_data["n_episodes"]
	is_converged[alpha_idx,lambda_idx,:,:] .= extracted_data["is_converged"]
	babbling_reward_r[alpha_idx,lambda_idx,:] .= babbling_nash["babbling_reward_r"] #CAN BE COMPUTED COMPUTE ONCE
end
...
for alpha_idx in 5:-1:1, lambda_idx in 1:1:5
	...
	pl_expected_reward_r = plot_dist(expected_reward_r[alpha_idx,lambda_idx,:,:];
									...
									width = raw"0.27\linewidth", height = raw"0.2\linewidth");
	pl_expected_reward_r = plot_val!(pl_expected_reward_r, best_nash["best_expected_reward_r"]; color = "red", style = "solid, semithick", opacity = 0.7);
	...
	pl_mutual_information = plot_dist(mutual_information[alpha_idx,lambda_idx,:,:];
									...
									additional = "ticks=none",
									width = raw"0.27\linewidth", height = raw"0.2\linewidth");
	pl_mutual_information = plot_val!(pl_mutual_information, best_nash["best_mutual_information"]; color = "red", style = "solid, semithick", opacity = 0.7);
	...
	push!(pls_expected_reward_r, pl_expected_reward_r)
	push!(pls_mutual_information, pl_mutual_information)
	push!(pls_is_gamma_nash, pl_is_gamma_nash)
end

group_pl_expected_reward_r = @pgf GroupPlot(
							{ group_style = { group_size="2 by 2", raw"horizontal sep = 5pt", raw"vertical sep = 5pt" },
   							 }, pls_expected_reward_r[[1,5,21,25]]...);

group_pl_mutual_information = @pgf GroupPlot(
							{ group_style = { group_size="2 by 2", raw"horizontal sep = 5pt", raw"vertical sep = 5pt" },
   							 }, pls_mutual_information[[1,5,21,25]]...);
...
plots = [
    	...
    	("grid_expected_reward_r", group_pl_expected_reward_r),
	]
